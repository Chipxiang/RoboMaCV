...
bool findRange(Mat int_frame)
{
	...
	vector<vector<Point> > contours_poly( contours.size() ); // added by mona
	...
	// changed by mona
	int count_rec_left = 0;
	for (int i = 0; i < leftRect.size() - 1; i++)
	{
		for (int j = 0; j != i && j < leftRect.size(); j++)
	...
	//bool change = judge_which_number(copy_frame(right_rect[0])); //commented by mona
	...
	/// added by mona
	Mat num_rects[9], num_rects_bw[9];
	for(int i=0; i<3; i++){
		for (int j=0; j<3; j++){
			Rect temp = Rect(roi_rect.tl().x + roi_rect.size().width/3*j, roi_rect.tl().y + roi_rect.size().height/3*i, roi_rect.size().width/3, roi_rect.size().height/3);
			num_rects[i*3+j] = copy_frame(temp);
			imshow("num_rects", num_rects[i*3+j]);
			//cvtColor(num_rects[i*3+j], num_rects_bw[i*3+j], CV_BGR2GRAY);
			threshold(num_rects[i*3+j], num_rects_bw[i*3+j], 200, 255, THRESH_BINARY);
			
			
			imshow("num_rects_bw", num_rects_bw[i*3+j]);
			waitKey(1);
		}
	}
	/// adding end
	...
	/*********第二个参数就是我要找的大神符数字************/
	match_number(roi, 8); // commented by mona
...
bool match_number(Mat find_roi, int number)
{
	...
	//imshow("roi", find_roi); // added by mona
